...
let cfg_of_thread thread_id { Program.body; _ } =
  ...
  (* The following functions take as a parameter and return a tuple
     `((graph, labels), offset)`. The graph is a CFG of
     a single-thread program, thus with edges in the form of [n :
     int].
...
let combine (cfg1, labels1) (cfg2, labels2) =
  (* Combines two CFG.
...
let of_program program =
  ...
  { program; graph; labels = Array.of_list labels }
