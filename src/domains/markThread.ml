  ...
  module Application = struct
    ...
    let generate_same_partitions
      ...
      StateAbstraction.M.Labels.fold
        ...
        ~init:(Interferences.inner_bot shared locals control)
        abstr

    let add_flush_x lbl tid shared locals control x key inner
        (acc_abstr, acc_intf) =
    ...
    let iterate_one_flush lbl tid shared locals control x abstr inner_intf =
      (* Does one iteration of flushing one x from each point of abstr if
         possible, adding the results to abstr *)
      StateAbstraction.M.Labels.fold
        ~f:(fun ~key ~data ->
            add_flush_x lbl tid shared locals control x key data)
        ~init:(abstr, inner_intf)
        abstr

    let close_by_flush_wrt_var lbl tid shared locals control x inner_intf abstr =
      let rec close acc_abstr acc_intf =
        let acc_abstr', acc_intf' =
          iterate_one_flush lbl tid shared locals control x acc_abstr acc_intf in
    ...
    let close_by_flush_wrt_varset lbl tid shared locals control varset_modified abstr =
      VarSet.fold
        (fun (ExistVar.Pack var) (acc_abstr, acc_intf) ->
           close_by_flush_wrt_var
             ...
             acc_intf acc_abstr)
        varset_modified
        (abstr, Interferences.inner_bot shared locals control)
    ...
    let apply lbl
      ...
      let abstr', generated_intf =
        ...
        |> StateAbstraction.M.map
          (apply_inner thread_id control shared locals inner_intf)
        |> close_by_flush_wrt_varset lbl thread_id shared locals control varset_intf in
    ...
    let generate_from_abstr op lbl1 lbl2 tid shared locals control abstr =
      ...
      | O.Identity ->
        ...
        generate_same_partitions
          lbl1 lbl2 tid shared locals control abstr (fun _key inner -> inner)
      | O.MFence ->
        let result =
